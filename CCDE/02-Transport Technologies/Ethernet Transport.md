...
Broadcast frames (DA of `FF:FF:FF:FF:FF:FF`) are always flooded out of all ports except the ingress port - there is no CAM lookup performed. Multicast frames are handled similarly by default (flooded to all ports), unless IGMP snooping is configured, in which case the switch tracks IGMP membership reports to build a multicast forwarding table and only forwards multicast frames to ports with interested receivers.
