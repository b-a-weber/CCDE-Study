...
The DA precedes the SA to allow for processing efficiencies. If the DA does not match the NIC's MAC address, then the frame can be dropped. If it does match the MAC address, then a new entry is added to the Content Addressable Memory (CAM) table to allow frames to be forwarded. In modern switches with purpose-built ASICs, switching is done at line rate using the CAM table without punting frames to the CPU. CAM is closely coupled with the ASIC - when data is provided to a switch's CAM, the comparison against existing MAC entries is done in parallel. Each row of the CAM table performs a simultaneous check against the data provided to the switch in a single clock cycle, with the appropriate match returning the switch port to forward the frame from. 

When a switch receives a frame, it performs two distinct operations on the SA and DA:

1. **Learning** - The SA is used to populate the CAM table, associating the source MAC with the ingress port. If the SA is already in the CAM table, the entry's timer is refreshed.
2. **Forwarding** - The DA is looked up in the CAM table to determine the egress port.

If the DA is not found in the CAM table (an _unknown unicast_), the switch floods the frame out of all ports except the ingress port. This is sometimes called unknown unicast flooding. Once the destination host responds, its SA is learned and subsequent frames can be forwarded directly rather than flooded.

Broadcast frames (`FF:FF:FF:FF:FF:FF`) are always flooded out of all ports except the ingress port - there is no CAM lookup performed. Multicast frames are handled similarly by default (flooded to all ports), unless IGMP snooping is configured, in which case the switch tracks IGMP membership reports to build a multicast forwarding table and only forwards multicast frames to ports with interested receivers.
...
The Ethernet frame concludes with the Frame Check Sequence (FCS) field. The FCS field is used for error detection - the frame is computed by the CRC-32 algorithm to generate a 4 byte representation of the frame. The receiver runs the same computation on the received frame and evaluates their value against the sender's FCS. If the FCS does not match, the Ethernet frame is dropped. There is no error correction or in-built reliability mechanism into FCS - if there is no match, the frame is dropped.
